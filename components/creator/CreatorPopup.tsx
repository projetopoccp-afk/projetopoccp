...
function getPlatformMetricLabel(t: (key: any) => string, platform: string) {
  const normalizedPlatform = platform.toLowerCase();
...
function getCreatorExternalReachFromLiveStatus(liveStatus: LiveStatusMap) {
  ...
  const discordMembers =
    discordStatus?.memberCount ?? discordStatus?.externalCount ?? 0;
...
function ViewPanel({
  ...
  const visibleClips = clips.filter((clip) =>
    Boolean(
      clip.url?.trim() || clip.thumbnailUrl?.trim() || clip.title?.trim(),
    ),
  );
  ...
  const rawProfileDescription =
    bio || creator.bio || description || creator.description || "";
  ...
  const isVerified = Boolean(
    (creator as any).isVerified ||
    (creator as any).verified ||
    (creator as any).is_verified ||
    (creator as any).verificationStatus === "verified" ||
    (creator as any).verification_status === "verified",
  );

  const isLiveOnAnyPlatform = Boolean(
    twitchStatus?.isLive || kickStatus?.isLive,
  );
    ...
    <>
      <div className="pr-16">
        <div className="mb-5 flex flex-wrap items-center gap-2">
          ...
          {isVerified && (
            <span className="inline-flex items-center gap-1.5 rounded-full border border-yellow-300/25 bg-yellow-300/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.18em] text-yellow-100">
              <ShieldCheck size={12} />
      ...
      <Link
        ...
        onClick={onClose}
        prefetch
        className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl border border-cyan-300/25 bg-cyan-300/10 px-5 py-3 text-sm font-black uppercase tracking-[0.18em] text-cyan-100 transition hover:border-cyan-200/50 hover:bg-cyan-300/20"
      >
      ...
      {socialLinkEntries.length > 0 && (
        <div className="mt-7 pb-10">
          ...
          >
            ...
            <ChevronDown
              size={18}
              className={`text-cyan-100 transition ${socialLinksOpen ? "rotate-180" : ""}`}
            />
          </button>
              ...
              <div className="grid gap-2">
                {socialLinkEntries.map(([platform, url]) => {
                  ...
                  const platformExternalCount =
                    getLiveStatusExternalCount(platformLiveStatus);
                  const platformExternalLabel = getPlatformMetricLabel(
                    t,
                    normalizedPlatform,
                  );
                  const platformName =
                    getPlatformDisplayName(normalizedPlatform);
                  ...
                  if (normalizedPlatform === "youtube") {
                    return (
                      <button
                        ...
                        className="flex min-w-0 items-center justify-between gap-3 rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-left transition hover:border-cyan-300/35 hover:bg-cyan-300/10"
                      >
                  ...
                  return (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className={`flex min-w-0 items-center justify-between gap-3 rounded-2xl border px-4 py-3 transition hover:scale-[1.01] ${
                        isPlatformLive
                          ? "border-emerald-300/25 bg-emerald-300/10 hover:bg-emerald-300/20"
                          : "border-white/10 bg-black/20 hover:border-cyan-300/35 hover:bg-cyan-300/10"
                      }`}
                    >
                      <span className="min-w-0">
                        <span className="block truncate text-sm font-black text-white">
...
function InfoCard({
  ...
  return (
    <div className="min-w-0 rounded-2xl border border-white/10 bg-white/[0.04] p-4">
      <p className="break-words text-[10px] uppercase leading-relaxed tracking-[0.14em] text-white/35">
        {label}
      </p>
